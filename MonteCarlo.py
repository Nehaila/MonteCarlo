...
class playmaze:
	...
	def MonteCarloEV(self):
			...
			for i in range(10):
				...
				ep_s=[]
				St=[]
				ep_s=[ep[i] for i in range(len(ep))]
				G=0
				for step in list(reversed(ep_s)):
					G=G*0.9+step[2]
					St.append(step[0])
					Returns[step[0][0],step[0][1]]= G
					V[step[0][0],step[0][1]].append(G)
			for e in list(V.keys()):
				if e not in list(Returns.keys()):
					del V[e]
				else:
					V[e]=np.average([j for j in V[e]])
			return V


		def MonteCarloFV(self):
		Returns={}
		V={}
		for s in self.States():
			V[s]=[]
